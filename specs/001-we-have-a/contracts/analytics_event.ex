defmodule WhoThere.AnalyticsEvent do
  @moduledoc """
  ...
  actions do
    ...
    create :create do
      ...
      change fn changeset, _opts ->
        ...
        if is_nil(device_type) and not is_nil(user_agent) do
          detected_type = WhoThere.DeviceDetector.detect(user_agent)
      ...
      change fn changeset, _opts ->
        ...
        if not is_nil(user_agent) and WhoThere.BotDetector.is_bot?(user_agent) do
          bot_name = WhoThere.BotDetector.identify_bot(user_agent)

          changeset
          |> Ash.Changeset.change_attribute(:event_type, :bot_traffic)
          |> Ash.Changeset.change_attribute(:bot_name, bot_name)
        else
          changeset
        end
      end
    ...
    read :by_event_type do
      ...
      prepare fn query, _context ->
        event_type = Ash.Query.get_argument(query, :event_type)
    ...
    read :bot_traffic_summary do
      ...
      prepare fn query, _context ->
        ...
        |> Ash.Query.sort(total_events: :desc)
      end
    end
    ...
    read :page_analytics do
      ...
      prepare fn query, _context ->
        ...
        if path_pattern do
          Ash.Query.filter(query, contains(path, ^path_pattern))
        else
          query
        end
      end
  ...
  validations do
    ...
    validate numericality(:status_code, greater_than_or_equal_to: 100, less_than_or_equal_to: 599),
  ...
  relationships do
    belongs_to :session, WhoThere.Session do
  ...
  aggregates do
    count :session_event_count, :session, :events do
      description "Number of events in the associated session"
    end
  end
